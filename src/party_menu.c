...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    // If player has 6th badge FlagGet(FLAG_BADGE06_GET) && , action list consists of < 4 moves, Mon can learn Fly, and player has HM02 in bag
    if (sPartyMenuInternal->numActions < 5 && (CanTeachMove(&mons[slotId], MOVE_FLY) != 1) && CheckBagHasItem(ITEM_HM02, 1)) 
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);

    // If player has 2nd badge FlagGet(FLAG_BADGE02_GET) && , action list consists of < 4 moves, Mon can learn Flash, and player has HM05 in bag
    if (sPartyMenuInternal->numActions < 5 && (CanTeachMove(&mons[slotId], MOVE_FLASH) != 1) && CheckBagHasItem(ITEM_HM05, 1)) 
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 1 + MENU_FIELD_MOVES);

    // If player has 1st badge FlagGet(FLAG_BADGE01_GET) && , action list consists of < 4 moves, Mon can learn Cut, and player has HM01 in bag
    if (sPartyMenuInternal->numActions < 5 && (CanTeachMove(&mons[slotId], MOVE_CUT) != 1) && CheckBagHasItem(ITEM_HM01, 1)) 
